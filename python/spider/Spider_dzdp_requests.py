...
class SpiderDZDP():
    ...
    def index_page(self, page):
        ...
        try:
            ...
            if(response.status_code == 200):
                self.get_shop_redirect_url(response.text)
            else:
                # 请求不到，可以更换更换代理IP，重试
    ...
    def get_shop_redirect_url(self, html):
        """
        获取当前页面跳转详情页集合
        :return:
        """
        ...
        for item in items:
            ...
            if single_shop_response.status_code == 200:
                ...
                # 获取店铺信息
                self.get_shop_info(single_shop_response.text)
                # 获取评论信息
                ...
                print("请求不到，可以更换更换代理IP，重试")
            time.sleep(1)
    def pattern(self, re_str, s):
        return re.compile(re_str, s)
    ...
    def get_shop_info(self, html):
        ...
        shopInfo = {
            ...
            "tasteScore": self.fetch_info(["spider_dzdp_num"], re.findall(self.pattern(self.taste_score_re, re.S), html)),
    ...
    def get_dictionary(self, url):
        time.sleep(2)
        dict_response = requests.get(url, headers=self.entry_headers)
        if dict_response.status_code == 200:
            html = dict_response.text
            # 拿到含有字体的css_url
            ...
            if woff_html_response.status_code == 200:
                ...
                for key in woff_name_url.keys():
                    ...
                    # 放入redis
                    RedisClient().setRedis(redisKey, ','.join(font_keys[2:]), 60 * 60 * 24 * 12)
                    # 删除字体文件
            ...
            print("获取字体链接失败 status_code=%s" % dict_response.status_code)
