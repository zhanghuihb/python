...
html_file = io.open('../spider/html.txt',encoding = 'utf-8')
html = html_file.read()

doc = PyQuery(html)
items = doc("#shop-all-list ul li").items()
address_re = '.*?<span class="addr">(.*?)</span>'

basefont_char = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '店', '中', '美', '家', '馆', '小', '车', '大',
                              '市',
                              '公', '酒',
...
for item in items:
    addr = item.find(".txt .tag-addr .addr").html(),
    print(addr)
